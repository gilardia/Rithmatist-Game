...
namespace Rithmatist.Networking
{
    public class Server
    {
        ...
        public Server(int port)
        {
            ...
            client = new Client[Rithmatist.Properties.Rithmatist.Default.MaxNumberOfClients];
        ...
        private void listener_userAdded(object sender, Client user)
        {
            ...
            if (Rithmatist.Properties.Rithmatist.Default.SendMessageToClientsWhenAUserIsAdded)
            {
                writeStream.Position = 0;

                //Write in the form {Protocol}{User_ID}{User_IP}
                writer.Write(Rithmatist.Properties.Rithmatist.Default.NewPlayerByteProtocol);
        ...
        private void user_UserDisconnected(object sender, Client user)
        {
            ...
            if (Rithmatist.Properties.Rithmatist.Default.SendMessageToClientsWhenAUserIsRemoved)
            {
                writeStream.Position = 0;

                //Write in the form {Protocol}{User_ID}{User_IP}
                writer.Write(Rithmatist.Properties.Rithmatist.Default.DisconnectedPlayerByteProtocol);
                writer.Write(user.id);
                writer.Write(user.IP);
        ...
        private void user_DataReceived(Client sender, byte[] data)
        {
            ...
            if (Rithmatist.Properties.Rithmatist.Default.EnableSendingIPAndIDWithEveryMessage)
            {
            ...
            if (Rithmatist.Properties.Rithmatist.Default.SendBackToOriginalClient)
            {                
                SendData(data);
            }
            else
